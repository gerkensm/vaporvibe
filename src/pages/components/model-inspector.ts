...
const COMPOSITE_SCORE_DESCRIPTIONS: Record<keyof ModelCompositeScores, string> =
  {
    reasoning:
...
const COMPOSITE_SCORE_LABELS: Record<keyof ModelCompositeScores, string> = {
  reasoning: "Reasoning",
...
type CapabilityState = "enabled" | "disabled" | "unknown";
type CapabilityVariant = "reasoning" | "images";
type CapabilityStates = Record<CapabilityVariant, CapabilityState>;
...
function renderCompositeScoreItems(scores?: ModelCompositeScores): string {
  if (!scores) {
    return '<p class="model-scores__empty">Composite scores are available for curated models.</p>';
  }
  const items = (
    Object.keys(COMPOSITE_SCORE_LABELS) as Array<keyof ModelCompositeScores>
  )
    .map((key) => {
      ...
      return `
        <div
          ...
          aria-label="${escapeHtml(
            `${label}: ${descriptor} (${displayValue} / 100)`
          )}"
...
interface CapabilityIndicatorConfig {
  readonly icon: string;
  readonly label: string;
  readonly description: string;
  readonly state: CapabilityState;
  readonly variant: CapabilityVariant;
}

function computeCapabilityStates(
  provider: ModelProvider,
  metadata?: ModelMetadata
): Record<CapabilityVariant, CapabilityState> {
  const providerCapabilities = PROVIDER_REASONING_CAPABILITIES[provider] ?? {
    mode: false,
    tokens: false,
  };
  const providerSupportsReasoning =
    providerCapabilities.mode === true || providerCapabilities.tokens === true;
  if (!metadata) {
    return {
  ...
  const tokensValue = Object.prototype.hasOwnProperty.call(
    metadata,
    "reasoningTokens"
  )
  ...
  const providerTokensAvailable =
    providerCapabilities.tokens === true && !tokensExplicitlyDisabled;
  const providerModesAvailable =
    providerCapabilities.mode === true && !modeExplicitlyDisabled;

  const hasReasoning =
    costIncludesReasoning ||
    tokensAvailable ||
    modeExplicitlySupported ||
    providerTokensAvailable ||
    providerModesAvailable;

  const reasoningState: CapabilityState = hasReasoning
    ? "enabled"
...
function renderCapabilityIndicator(config: CapabilityIndicatorConfig): string {
...
const CAPABILITY_ICON_MARKUP: Record<CapabilityVariant, string> = {
  reasoning:
    '<svg class="model-capability__svg" viewBox="0 0 20 20" aria-hidden="true" focusable="false"><circle cx="4" cy="5" r="2" fill="currentColor"/><circle cx="16" cy="5.5" r="2" fill="currentColor"/><circle cx="10" cy="14.5" r="2.1" fill="currentColor"/><path d="M5.6 6.7 8.9 11m5.3-3.2-3.8 6.2M12.2 4.2l-4 .8" stroke="currentColor" fill="none"/></svg>',
  images:
    '<svg class="model-capability__svg" viewBox="0 0 20 20" aria-hidden="true" focusable="false"><rect x="2.5" y="3.5" width="15" height="11" rx="2" stroke="currentColor" fill="none"/><circle cx="7.3" cy="8" r="1.6" fill="currentColor"/><path d="M5.2 13.3 8.4 10l2.7 2.9 2.1-1.9 2.6 2.3" stroke="currentColor" fill="none"/></svg>',
};
...
export function renderModelDetailPanel(
  ...
  if (!metadata) {
    return `<div class="model-detail" data-model-detail>
      ...
      <div class="model-capabilities" data-model-capabilities>${renderCapabilities(
        provider
      )}</div>
  ...
  return `<div class="model-detail" data-model-detail>
    <div class="model-detail__header">
      <div>
        ...
        <p class="model-detail__tagline" data-model-tagline>${escapeHtml(
          metadata.tagline ?? ""
        )}</p>
      </div>
      <div class="model-detail__meta">
        <div class="model-detail__cost" data-model-cost>${escapeHtml(
          costDisplay
        )}</div>
      </div>
    </div>
    ...
    <div class="model-scores" data-model-scores>${renderCompositeScoreItems(
      metadata.compositeScores
    )}</div>
    <dl class="model-detail__facts">
      <div><dt>Context window</dt><dd data-model-context>${escapeHtml(
        context
      )}</dd></div>
      ...
      <div><dt>Release</dt><dd data-model-release>${escapeHtml(
        metadata.release ?? "—"
      )}</dd></div>
    </dl>
  </div>`;
