...
function renderProviderStep(options: ProviderStepOptions): string {
  ...
  const explicitMaxOutputValue =
    respectStoredMaxOutput && sanitizedMaxOutput !== maxOutputDefault
      ? sanitizedMaxOutput
      : null;
  ...
  const currentReasoningTokens =
    reasoningToggleEnabled && typeof reasoningTokens === "number"
      ? reasoningTokens
      : null;
  ...
  const maxTokensControlMarkup = renderTokenBudgetControl({
    ...
    description:
      maxOutputRange.description?.trim().length
  ...
  return `<section class="card">
    ...
    keyStatusVariant
  )}" data-selection-required="${providerSelectionRequired ? "true" : "false"}" data-initial-reasoning-enabled="${
...
function renderProviderScript(
  ...
  selectedModel: string,
  reasoningMode: ReasoningMode,
  reasoningTokensEnabled: boolean | undefined,
  _reasoningTokens: number | undefined,
  ...
  const providerDefaultsJson = JSON.stringify(
    DEFAULT_MAX_TOKENS_BY_PROVIDER,
  ).replace(/</g, "\u003c");
  ...
  const tokenGuidanceJson = JSON.stringify(PROVIDER_TOKEN_GUIDANCE).replace(
    /</g,
    "\u003c",
  );
