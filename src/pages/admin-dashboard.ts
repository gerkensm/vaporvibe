  ...
  const items = history
    .map((item, index) => {
      ...
      if (item.attachments?.length) {
        metaRows.push(
          `<div class="history-meta-row"><span>Attachments</span><strong>${escapeHtml(
            formatAttachmentMeta(item.attachments)
          )}</strong></div>`
        );
      }
...
function renderBriefAttachmentGallery(
  attachments: BriefAttachment[]
): string {
  ...
  return attachments
    .map((attachment) => {
      ...
      return `<div class="attachment-card" data-attachment-id="${escapeHtml(
        attachment.id
      )}">
  ...
  <button type="button" data-remove-attachment="${escapeHtml(
    attachment.id
  )}">Remove</button>
</div>`;
    })
...
function renderHistoryAttachments(
  ...
  const items = attachments.map((attachment) => {
    const label = `${attachment.name} (${attachment.mimeType}, ${formatBytes(
      attachment.size
    )})`;
    if (attachment.mimeType.toLowerCase().startsWith("image/")) {
      return `<figure class="history-attachment"><img src="${createAttachmentDataUrl(
        attachment
      )}" alt="${escapeHtml(attachment.name)}" loading="lazy" /><figcaption>${escapeHtml(
        label
      )}</figcaption></figure>`;
    }
    return `<div class="history-attachment">
  <p>${escapeHtml(label)}</p>
  <a href="${createAttachmentDataUrl(attachment)}" download="${escapeHtml(
      attachment.name
    )}">Download</a>
</div>`;
  });
...
function formatAttachmentMeta(attachments: BriefAttachment[]): string {
  if (!attachments || attachments.length === 0) {
    return "No files";
  }
...
function createAttachmentDataUrl(attachment: BriefAttachment): string {
