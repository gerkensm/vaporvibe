...
export function renderAdminDashboard(props: AdminPageProps): string {
  ...
  const modelOptions = getModelOptionList(
    providerKey,
    provider.model || defaultModel,
  );
  const modelDatalistId = `${modelInputId}-options`;
  const modelDatalist = `<datalist id="${escapeHtml(
    modelDatalistId,
  )}">${modelOptions
    .map(
      (option) =>
    ...
    .join("\n")}</datalist>`;
  const modelDetailPanel = renderModelDetailPanel(
    providerKey,
    provider.model || defaultModel,
  );
  const modelLineup = renderModelLineup(
    providerKey,
    provider.model || defaultModel,
  );
