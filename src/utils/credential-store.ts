...
export class CredentialStore {
  ...
  private async loadKeytar(): Promise<boolean> {
    ...
    try {
      ...
      console.warn(
        "Secure credential storage unavailable (keytar not loaded). " +
  ...
  async saveApiKey(provider: ModelProvider, apiKey: string): Promise<void> {
    ...
    if (await this.loadKeytar()) {
      try {
        ...
        console.error(
          `Failed to save ${provider} credential to keychain:`,
  ...
  async getApiKey(provider: ModelProvider): Promise<string | null> {
    ...
    if (await this.loadKeytar()) {
      try {
        ...
        console.error(
          `Failed to retrieve ${provider} credential from keychain:`,
          error instanceof Error ? error.message : String(error)
        );
      }
    }
  ...
  async deleteApiKey(provider: ModelProvider): Promise<boolean> {
    ...
    if (await this.loadKeytar()) {
      try {
        ...
        console.error(
          `Failed to delete ${provider} credential from keychain:`,
          error instanceof Error ? error.message : String(error)
        );
        return false;
      }
