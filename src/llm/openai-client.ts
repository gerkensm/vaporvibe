  ...
  async generateHtml(
    ...
    stream.on("response.output_text.delta", (event: any) => {
      if (typeof event?.snapshot === "string") {
        const previousLength = streamedHtml.length;
        streamedHtml = event.snapshot;
        const added = streamedHtml.slice(previousLength);
        tokenTracker.addFromText(added);
      } else if (typeof event?.delta === "string") {
        streamedHtml += event.delta;
        tokenTracker.addFromText(event.delta);
      }
    });
    ...
    const completionTotal =
      usage?.outputTokens !== undefined || usage?.reasoningTokens !== undefined
        ...
        : undefined;
    tokenTracker.finalize(completionTotal);
...
function extractUsageMetrics(response: any): LlmUsageMetrics | undefined {
  ...
  const output =
    usage.output_tokens ?? usage.output_token_count ?? usage.completion_tokens;
