...
export function buildMessages(context: MessageContext): ChatMessage[] {
  ...
  const systemLines = isJsonQuery
    ...
    : [
        ...
        "9) Component & style reuse.",
        '   - Previous HTML includes *server-injected* data-id attributes (e.g., data-id="sl-gen-3") for the <html>/<head>/<body> shells, major structural regions, inline <script> helpers, and data-style-id attributes on <style> tags.',
        "   - Never invent, apply, or copy over these data-id/data-style-id attributes yourself—the server applies them for you.",
        "   - PRIORITY: Look if the same page was rendered before in your history. If no adaption needs to be made, you can output only the {{component:slug-of-the-html-tag}} and that's all you need to do to serve a 100% copy of that page. Reach first for ids that appear in the latest 'Previous HTML' block—treat them as canonical for chrome, wrappers, and static scripts.",
        "   - Replace any unchanged structural element (headers, footers, nav, hero shells, layout wrappers) or static helper <script> by emitting {{component:<data-id>}}; the server restores the full element before streaming.",
        "   - Split inline JS so durable helpers live in their own `<script>` tag (the server will stamp the data-id), and put request-specific state in a short `<script data-state-script>` (or similar). Re-render the state script every time and never reference it via a placeholder.",
        "   - If a global <style> block is unchanged, output {{style:<data-style-id>}} as a standalone node inside <head> (no additional wrapper).",
        "   - Historical reuse is advanced. Only reference ids from earlier history entries when you deliberately need to revive a component the latest page no longer contains.",
        "       - When resurfacing a previously generated static page, you can reuse either the full html (using the same placeholder technique) if nothing on the page must be adapted, or more selectively the body, the head, or any of its cached components individually, to reduce latency—as long as no part of it needs to change, even if it's not the previous page but one longer ago in the history.",
        "   - Never reuse components or styles for areas containing dynamic data, user-submitted content, or sections the user just modified, and ensure the resolved document is still a full <html>…</html> page.",
        "",
  ...
  const rememberLines = isJsonQuery
    ...
    : [
        ...
        "- When you reuse persistent chrome, grab ids from the immediate Previous HTML first. Only pull from older history if you rendered that exact page before or when you purposely need to revive a missing component, and reference it via {{component:<data-id>}} or {{style:<data-style-id>}} (style placeholders appear standalone in <head>).",
