...
function resolveOptions(options = {}) {
    const message = typeof options.message === "string" && options.message.trim().length > 0
        ? options.message
        : DEFAULT_STATUS_MESSAGE;
    const accent = typeof options.accent === "string" && options.accent.trim().length > 0
        ? options.accent
        : DEFAULT_ACCENT;
    const originalPath = typeof options.originalPath === "string" && options.originalPath.trim().length > 0
        ? options.originalPath
        : DEFAULT_ORIGINAL_PATH;
    const resultRoutePrefix = typeof options.resultRoutePrefix === "string" &&
...
function renderHydrationScript(resultRoutePrefix, originalPath) {
...
export function renderLoadingShell(options = {}) {
    ...
    const hydrationScript = renderHydrationScript(resolved.resultRoutePrefix, resolved.originalPath);
    return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8" />
