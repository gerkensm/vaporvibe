...
async function main() {
    try {
        ...
        const portExplicitlySpecified = cliOptions.port !== undefined || process.env.PORT !== undefined;
        const actualPort = portExplicitlySpecified
            ? await startServerOnExactPort(server, appConfig.runtime.port, appConfig.runtime.host)
            : await startServerWithPortFallback(server, appConfig.runtime.port, appConfig.runtime.host);
        ...
        logger.info({ port: actualPort, host: appConfig.runtime.host, url: localUrl }, `Sourcecodeless server ready at ${localUrl}`);
...
async function startServerOnExactPort(server, port, host) {
...
async function startServerWithPortFallback(server, preferredPort, host) {
    ...
    for (let port = preferredPort; port <= MAX_PORT; port += 1) {
        try {
            ...
            if (error instanceof Error &&
                ...
                if (port === MAX_PORT) {
                    throw new Error(`No available ports found in range ${preferredPort}-${MAX_PORT}`);
                }
                continue;
            }
            throw error;
    ...
    throw new Error(`Failed to start server on any port in range ${preferredPort}-${MAX_PORT}`);
}
function printHelp() {
