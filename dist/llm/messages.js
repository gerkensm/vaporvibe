...
export function buildMessages(context) {
    const { brief, briefAttachments, omittedAttachmentCount, method, path, query, body, prevHtml, timestamp, includeInstructionPanel, history, historyTotal, historyLimit, historyMaxBytes, historyBytesUsed, historyLimitOmitted, historyByteOmitted, adminPath, restMutations, restQueries, mode = "page", } = context;
    ...
    const insertIndex = realisticContentIndex === -1
        ? systemLines.length
    ...
    if (!isJsonQuery) {
        ...
        if (includeInstructionPanel) {
            iterationLines.push("13) RUNTIME PANEL: The server injects the floating instructions widget—do not render your own version or duplicate its controls.");
        }
    ...
    userSections.push("", "Current Request:", `- Timestamp: ${nowIso}`, `- Method: ${method}`, `- Path: ${path}`, `- Query Params (URL-decoded JSON): ${JSON.stringify(query, null, 2)}`, `- Body Params (URL-decoded JSON): ${JSON.stringify(body, null, 2)}`, "", "Previous HTML (for context; may be empty if first view):", "-----BEGIN PREVIOUS HTML-----", prevHtmlSnippet, "-----END PREVIOUS HTML-----");
    if (restMutations.length > 0) {
        userSections.push("", "Recorded Mutations (latest first):", ...summarizeRestRecords(restMutations));
    }
    if (restQueries.length > 0) {
        userSections.push("", "Recorded Query Results (latest first):", ...summarizeRestQueries(restQueries));
    }
    userSections.push("", historySection, "", "Remember:", ...(isJsonQuery
        ? [
...
function summarizeRestRecords(records) {
    return records
        .slice()
        .reverse()
        .flatMap((record) => {
...
function summarizeRestQueries(records) {
    return records
        .slice()
        .reverse()
        .flatMap((record) => {
...
function truncateMultiline(value, maxLength = 400) {
