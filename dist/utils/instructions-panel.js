...
export function getInstructionsPanelScript() {
    ...
    const configScript = `window.__SERVE_LLM_INSTRUCTIONS_CONFIG__ = ${JSON.stringify(config)};`;
    const { url, mode } = resolveScriptSource("instructions-panel.js", "/src/instructions-panel.ts");
    const loaderTag = mode === "asset"
