...
function formatTokenDisplay(value, units) {
...
export function renderTokenBudgetControl(options) {
    const { id, name, label, description, helper, value, defaultValue, min, max, step, units = "tokens", allowBlank = true, sliderEnabled = true, disabled = false, emptyLabel = "Provider default", defaultLabel = "Provider default", specialLabels = {}, accent, manualPlaceholder = "Enter an exact value", } = options;
    ...
    const specialLabelEntries = Object.entries(specialLabels).map(([key, val]) => `\"${escapeHtml(key)}\":\"${escapeHtml(val)}\"`);
    ...
    data-token-default="${effectiveDefault !== null && effectiveDefault !== undefined
...
export const TOKEN_BUDGET_RUNTIME = `(() => {
  ...
  const init = (root, config = {}) => {
    ...
    const applyValue = (rawValue, source) => {
      ...
      if (slider instanceof HTMLInputElement) {
        if (!Number.isFinite(numeric)) {
          const defaultNumeric = Number(state.defaultValue || state.min || 0);
          slider.value = String(clamp(defaultNumeric, state.min, state.max));
          sliderInRange = false;
        } else {
          const clamped = clamp(numeric, state.min, state.max);
          slider.value = String(clamped);
          sliderInRange = clamped === numeric;
        }
      }
      if (badge instanceof HTMLElement) {
        badge.textContent = formatBadge(normalized);
      }
      root.dataset.sliderOutside = sliderInRange ? "false" : "true";
