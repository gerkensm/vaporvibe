...
VaporVibe includes a **Standard Library** of ~60 popular UI, animation, data visualization, and utility libraries that are available offline. These libraries are automatically managed, version-detected, and presented to the LLM to ensure consistent and high-quality generation.
...
| **GSAP**            | GreenSock animation platform (`gsap.to('.box', {x: 100})`)                 |
| **Motion**          | Modern animation library (`animate('.box', {opacity: 1})`)                 |
| **Lottie Web**      | After Effects animations (`lottie.loadAnimation({...})`)                   |
| **tsParticles**     | Particle effects engine (`tsParticles.load('id', {...})`)                  |
| **Vanta.js**        | Animated 3D backgrounds (`VANTA.WAVES({el: '#bg'})`)                       |
| **Lenis**           | Smooth scroll library (`new Lenis()`)                                      |
| **AutoAnimate**     | Drop-in animations (`autoAnimate(parentElement)`)                          |
...
| Library      | Description                                      |
| ------------ | ------------------------------------------------ |
| **Barba.js** | Page transitions with PJAX (`barba.init({...})`) |
| **Swup**     | Animated page transitions (`new Swup()`)         |
...
| **ApexCharts** | Modern interactive charts (`new ApexCharts(...)`) |
| **ECharts**    | Apache data visualization (`echarts.init(...)`)   |
| **Cytoscape**  | Graph/network visualization (`cytoscape({...})`)  |
...
| **PhotoSwipe**  | Lightbox gallery (`new PhotoSwipe({...})`)                                 |
| **Medium Zoom** | Medium-style image zoom (`mediumZoom('img')`)                              |
...
| Library        | Description                                          |
| -------------- | ---------------------------------------------------- |
| **Editor.js**  | Block-style rich text editor (`new EditorJS({...})`) |
| **CodeMirror** | Code editor component (`new EditorView({...})`)      |
...
| **Uppy**              | File uploader with UI (`new Uppy()`)                 |
| **Floating UI**       | Tooltip/popover positioning (`computePosition(...)`) |
